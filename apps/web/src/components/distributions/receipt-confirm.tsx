...
export function ReceiptConfirm({
  ...
  const [success, setSuccess] = useState<ConfirmReceiptResponse['member'] | null>(null);
  ...
  async function submit(body: { token?: string; membershipId?: string }) {
    ...
    setSuccess(null);
    try {
      const res = await api.post<ConfirmReceiptResponse>(`/distributions/${distributionId}/receipts`, {
        ...body,
        allocationId,
      });
